...
for (line, expr) in Pair[
    "sin"          => :sin,
    "Base.sin"     => :(Base.sin),
    "@time(x)"     => Expr(:macrocall, Symbol("@time"), Expr(:line, 1, :none), :x),
    "@time"        => Expr(:macrocall, Symbol("@time"), Expr(:line, 1, :none)),
    ":@time"       => Expr(:quote, (Expr(:macrocall, Symbol("@time"), Expr(:line, 1, :none)))),
    "@time()"      => Expr(:macrocall, Symbol("@time"), Expr(:line, 1, :none)),
    "Base.@time()" => Expr(:macrocall, Expr(:., :Base, QuoteNode(Symbol("@time"))), Expr(:line, 1, :none)),
    "ccall"        => :ccall, # keyword
    "while       " => :while, # keyword, trailing spaces should be stripped.
    "0"            => 0,
    "\"...\""      => "...",
    "r\"...\""     => Expr(:macrocall, Symbol("@r_str"), Expr(:line, 1, :none), "...")
    ]
    @test Docs.helpmode(line) == Expr(:macrocall, Expr(:., Expr(:., :Base, QuoteNode(:Docs)), QuoteNode(Symbol("@repl"))), Expr(:line, 117, Symbol("docs/utils.jl")), STDOUT, expr)
    buf = IOBuffer()
...
@test Docs._repl(:(dynamic_test(1.0))) == Expr(:macrocall, Symbol("@doc"), Expr(:line, 204, Symbol("docs/utils.jl")), esc(:(dynamic_test(::typeof(1.0)))))
@test Docs._repl(:(dynamic_test(::String))) == Expr(:macrocall, Symbol("@doc"), Expr(:line, 204, Symbol("docs/utils.jl")), esc(:(dynamic_test(::String))))
